...
//        }
    ...
    private String replaceSchema(String str, DataMasking dataMasking) {
        ...
        int startInclude = dataMasking.startInclude(), endExclude = dataMasking.endExclude();
        if (dataMasking.maskingPercent() > NumberUtils.FLOAT_ZERO && dataMasking.maskingPercent() < NumberUtils.FLOAT_ONE) {
            int strLen = StringUtils.length(str);
            startInclude = (int) (strLen * (dataMasking.maskingPercent()));
            endExclude = (int) (strLen * dataMasking.maskingPercent() * 2);
        }
        if (StrUtil.length(dataMasking.masking()) > 1) {
            return StrUtil.replace(str, startInclude, endExclude, dataMasking.masking());
        }
        return StrUtil.replace(str, startInclude, endExclude, dataMasking.masking().charAt(0));
    }
